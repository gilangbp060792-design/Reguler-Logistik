...
const Fleet = () => {
  ...
  const initialForm = { plate: '', type: 'Truck', brand: '', capacity: '', year: new Date().getFullYear(), status: 'Active', driverId: '', mileage: 0 };
  const [form, setForm] = useState(initialForm);

  const openNew = () => { setEditing(null); setForm(initialForm); setShowModal(true); };
  const openEdit = (v) => { setEditing(v); setForm({ plate: v.plate, type: v.type, brand: v.brand, capacity: v.capacity, year: v.year, status: v.status, driverId: v.driverId || '', mileage: v.mileage }); setShowModal(true); };
  const close = () => { setShowModal(false); setEditing(null); };

  const handleSubmit = (e) => {
    e.preventDefault();
    const data = { ...form, year: Number(form.year), mileage: Number(form.mileage), driverId: form.driverId || null };
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        <div style={{ overflowX: 'auto' }}>
          <table className="data-table">
            <thead>
              <tr>
                <th>ID Kendaraan</th><th>Tipe</th><th>Merek / Model</th><th>Plat Nomor</th><th>Kapasitas</th><th>Tahun</th><th>Jarak Tempuh</th><th>Status</th><th>Pengemudi</th><th style={{ textAlign: 'center' }}>Aksi</th>
              </tr>
            </thead>
            <tbody>
              {fleet.map(v => {
                ...
                return (
                  <tr key={v.id}>
                    <td className="font-mono" style={{ color: 'var(--primary)' }}>{v.id}</td>
                    ...
                    <td>{v.capacity}</td>
                    <td>{v.year}</td>
                    <td>{v.mileage.toLocaleString('id-ID')} km</td>
        ...
        <div className="modal-overlay" onClick={close}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h3 className="font-headline-md">{editing ? `Edit ${editing.id}` : 'Tambah Kendaraan'}</h3>
            ...
            <form onSubmit={handleSubmit}>
              <div className="modal-body">
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px' }}>
                  ...
                  <div className="form-group"><label className="form-label">Jarak Tempuh (km)</label><input type="number" className="form-control" value={form.mileage} onChange={(e) => setForm({ ...form, mileage: e.target.value })} /></div>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
